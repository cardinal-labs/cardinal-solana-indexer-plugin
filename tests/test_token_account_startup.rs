...
fn test_token_account_startup() {
    ...
    assert!(pubkey == ADDRESS.to_string(), "Incorrect pubkey");
    let owner: String = first_row.get("owner");
    assert!(owner == TOKEN_ACCOUNT_OWNER.to_string(), "Incorrect pubkey");
    let mint: String = first_row.get("mint");
    assert!(mint == MINT.to_string(), "Incorrect pubkey");
